...
class UserRegistrationForm(forms.ModelForm):
    ...
    class Meta:
        model = User
        fields = ['first_name', 'last_name', 'email', 'phone', 'birth_date', 'password', 'confirm_password', 'gender', 'passport_id']
        widgets = {
    ...
    def clean_email(self):
        ...
        if email:
            try:
                ...
                raise forms.ValidationError("A user with this email already exists.")  # Handle exception gracefully
        return email
        ...
        if passport_id:
            try:
                if User.objects.filter(passport_id=passport_id).exists():
                    raise forms.ValidationError("A user with this passport ID already exists.")
            except Exception as e:
                print(f"Exception occurred: {e}")  # Add this line for debugging
                raise forms.ValidationError("A user with this passport ID already exists.")  # Handle exception gracefully
    ...
    def clean_confirm_password(self):
        ...
        if password and confirm_password and password != confirm_password:
            raise forms.ValidationError("Passwords do not match. Please enter the same password again.")
